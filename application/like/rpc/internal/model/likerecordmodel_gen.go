...
var (
	...
	cacheBehuTestLikeLikeRecordIdPrefix               = "cache:behuTestLike:likeRecord:id:"
	cacheBehuTestLikeLikeRecordBizIdObjIdUserIdPrefix = "cache:behuTestLike:likeRecord:bizId:objId:userId:"
)
...
func (m *defaultLikeRecordModel) Delete(ctx context.Context, id int64) error {
	...
	behuTestLikeLikeRecordBizIdObjIdUserIdKey := fmt.Sprintf("%s%v:%v:%v", cacheBehuTestLikeLikeRecordBizIdObjIdUserIdPrefix, data.BizId, data.ObjId, data.UserId)
	behuTestLikeLikeRecordIdKey := fmt.Sprintf("%s%v", cacheBehuTestLikeLikeRecordIdPrefix, id)
	_, err = m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
		return conn.ExecCtx(ctx, query, id)
	}, behuTestLikeLikeRecordBizIdObjIdUserIdKey, behuTestLikeLikeRecordIdKey)
	return err
}
...
func (m *defaultLikeRecordModel) FindOne(ctx context.Context, id int64) (*LikeRecord, error) {
	behuTestLikeLikeRecordIdKey := fmt.Sprintf("%s%v", cacheBehuTestLikeLikeRecordIdPrefix, id)
	var resp LikeRecord
	err := m.QueryRowCtx(ctx, &resp, behuTestLikeLikeRecordIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
...
func (m *defaultLikeRecordModel) FindOneByBizIdObjIdUserId(ctx context.Context, bizId string, objId int64, userId int64) (*LikeRecord, error) {
	behuTestLikeLikeRecordBizIdObjIdUserIdKey := fmt.Sprintf("%s%v:%v:%v", cacheBehuTestLikeLikeRecordBizIdObjIdUserIdPrefix, bizId, objId, userId)
	var resp LikeRecord
	err := m.QueryRowIndexCtx(ctx, &resp, behuTestLikeLikeRecordBizIdObjIdUserIdKey, m.formatPrimary, func(ctx context.Context, conn sqlx.SqlConn, v any) (i any, e error) {
...
func (m *defaultLikeRecordModel) Insert(ctx context.Context, data *LikeRecord) (sql.Result, error) {
	behuTestLikeLikeRecordBizIdObjIdUserIdKey := fmt.Sprintf("%s%v:%v:%v", cacheBehuTestLikeLikeRecordBizIdObjIdUserIdPrefix, data.BizId, data.ObjId, data.UserId)
	behuTestLikeLikeRecordIdKey := fmt.Sprintf("%s%v", cacheBehuTestLikeLikeRecordIdPrefix, data.Id)
...
func (m *defaultLikeRecordModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheBehuTestLikeLikeRecordIdPrefix, primary)
}
