...
var (
	...
	cacheBehuTestArticleArticleIdPrefix = "cache:behuTestArticle:article:id:"
)
...
func (m *defaultArticleModel) Delete(ctx context.Context, id int64) error {
	behuTestArticleArticleIdKey := fmt.Sprintf("%s%v", cacheBehuTestArticleArticleIdPrefix, id)
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
		return conn.ExecCtx(ctx, query, id)
	}, behuTestArticleArticleIdKey)
	return err
}
...
func (m *defaultArticleModel) FindOne(ctx context.Context, id int64) (*Article, error) {
	behuTestArticleArticleIdKey := fmt.Sprintf("%s%v", cacheBehuTestArticleArticleIdPrefix, id)
	var resp Article
	err := m.QueryRowCtx(ctx, &resp, behuTestArticleArticleIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
...
func (m *defaultArticleModel) Insert(ctx context.Context, data *Article) (sql.Result, error) {
	behuTestArticleArticleIdKey := fmt.Sprintf("%s%v", cacheBehuTestArticleArticleIdPrefix, data.Id)
...
func (m *defaultArticleModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheBehuTestArticleArticleIdPrefix, primary)
}
