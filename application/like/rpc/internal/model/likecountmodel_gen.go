...
var (
	...
	cacheBehuTestLikeLikeCountIdPrefix         = "cache:behuTestLike:likeCount:id:"
	cacheBehuTestLikeLikeCountBizIdObjIdPrefix = "cache:behuTestLike:likeCount:bizId:objId:"
)
...
func (m *defaultLikeCountModel) Delete(ctx context.Context, id int64) error {
	...
	behuTestLikeLikeCountBizIdObjIdKey := fmt.Sprintf("%s%v:%v", cacheBehuTestLikeLikeCountBizIdObjIdPrefix, data.BizId, data.ObjId)
	behuTestLikeLikeCountIdKey := fmt.Sprintf("%s%v", cacheBehuTestLikeLikeCountIdPrefix, id)
	_, err = m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
		return conn.ExecCtx(ctx, query, id)
	}, behuTestLikeLikeCountBizIdObjIdKey, behuTestLikeLikeCountIdKey)
	return err
}
...
func (m *defaultLikeCountModel) FindOne(ctx context.Context, id int64) (*LikeCount, error) {
	behuTestLikeLikeCountIdKey := fmt.Sprintf("%s%v", cacheBehuTestLikeLikeCountIdPrefix, id)
	var resp LikeCount
	err := m.QueryRowCtx(ctx, &resp, behuTestLikeLikeCountIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
...
func (m *defaultLikeCountModel) FindOneByBizIdObjId(ctx context.Context, bizId string, objId int64) (*LikeCount, error) {
	behuTestLikeLikeCountBizIdObjIdKey := fmt.Sprintf("%s%v:%v", cacheBehuTestLikeLikeCountBizIdObjIdPrefix, bizId, objId)
	var resp LikeCount
	err := m.QueryRowIndexCtx(ctx, &resp, behuTestLikeLikeCountBizIdObjIdKey, m.formatPrimary, func(ctx context.Context, conn sqlx.SqlConn, v any) (i any, e error) {
...
func (m *defaultLikeCountModel) Insert(ctx context.Context, data *LikeCount) (sql.Result, error) {
	behuTestLikeLikeCountBizIdObjIdKey := fmt.Sprintf("%s%v:%v", cacheBehuTestLikeLikeCountBizIdObjIdPrefix, data.BizId, data.ObjId)
	behuTestLikeLikeCountIdKey := fmt.Sprintf("%s%v", cacheBehuTestLikeLikeCountIdPrefix, data.Id)
...
func (m *defaultLikeCountModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheBehuTestLikeLikeCountIdPrefix, primary)
}
